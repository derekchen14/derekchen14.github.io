...
The closest thing to a proactive measure is Cortex Code's reflection step, where the model reviews its work after each tool call and chooses to clarify, iterate, or finalize. Assistant Factory is the only framework treating ambiguity as a first-class concern, classifying uncertainty into a four-level gradient running from general (the intent is unclear) through partial (the key entity is ungrounded) and specific (a required value is missing) to confirmation (a candidate needs sign-off), with each level carrying its own response strategy, so the agent can react accordingly. Choosing the right sub-agent comes from an ensemble of multiple models voting, which offers a confidence score on routing decisions.[^21] These components allow the agent to obtain a theory of mind, and build a deep understanding of the user's intent, not found in any other SDK.
...
| **Assistant Factory** | Deterministic shell, model in Skills | Stack of Flows | Four-level ambiguity gradient | Three-tier typed cache | Signal envelopes plus three-level evals |
